...
class RBX2SR_nores(nn.Module):
    def __init__(self, inp_planes, out_planes, depth_multiplier, deploy_flag = False):
        super(RBX2SR_nores, self).__init__()
...
if __name__ == '__main__':
    ...
    RBX2SR = RBX2SR(16, 16, 4)
    y0 = RBX2SR(x)

    RK, RB = RBX2SR.rep_params()
    y1 = F.conv2d(input=x, weight=RK, bias=RB, stride=1, padding=1)
    print('RBX2SR after reparameterization: ', torch.mean(torch.abs(y0-y1)))