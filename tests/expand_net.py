...
res = compute_ck(model_linear[0], model_linear[1])
...
fused = torch.nn.Conv2d(
    ...
    kernel_size=squeezenet1_1.features[3].expand3x3.kernel_size,
    stride=squeezenet1_1.features[3].expand3x3.stride,
    padding=squeezenet1_1.features[3].expand3x3.padding,
    bias=True
)
...
res = compute_cl(model_linear[0], model_linear[1])
fused.weight.data = res['weight']
fused.bias.data = res['bias']
f2 = fused.forward(input)
d = (f1 - f2).sum().item()
...
ipdb> squeezenet1_1.features[3].expand3x3.out_channels
'''