...
class UnpairedLMDBDataset_single(BaseDataset):
    def __init__(self, data_opt, **kwargs):
        """ LMDB dataset with unpaired data, for BD degradation
        """
        super(UnpairedLMDBDataset_single, self).__init__(data_opt, **kwargs)
    ...
    def __getitem__(self, item):
        ...
        frms_ = []
        for frm in frms:
            frm = frm.transpose(1, 2, 0)  # hwc
            frm = np.uint8(np.clip(np.round(frm * 255), 0, 255))

            # convert to YCrCb (cv2 reads images in BGR!), and normalize
            frm = cv2.cvtColor(frm, cv2.COLOR_BGR2YCrCb)
            # only work on the luminance channel Y
            frm = frm[:,:,0:1]
            frm = frm.transpose(2, 0, 1)  # chw|Y|uint8
            frms_.append(frm)

        #print('frms previous ', len(frms), len(frms[0]) ,len(frms[0][0]),len(frms[0][0][0]))
        #print('frms after ', len(frms_), len(frms_[0]) ,len(frms_[0][0]),len(frms_[0][0][0]))
        frms = frms_
        frms = np.stack(frms)  # tchw|rgb|uint8
